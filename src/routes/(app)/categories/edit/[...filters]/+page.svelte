<script lang="ts">
    import CategoriesEdit from "$lib/pages/categories/CategoriesEdit.svelte";
    ...
    const storedProvider = localStorage?.getItem('temp-categories-values');
    if (storedProvider) {
        values = JSON.parse(storedProvider);
        localStorage.removeItem('temp-categories-values');
    }
...
<CategoriesEdit {tempId} {filterName} {filterValue} values={values}  />