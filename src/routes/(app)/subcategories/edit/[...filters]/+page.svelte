<script lang="ts">
    import SubcategoriesEdit from "$lib/pages/subcategories/SubcategoriesEdit.svelte";
    ...
    const storedProvider = localStorage?.getItem('temp-subcategories-values');
    if (storedProvider) {
        values = JSON.parse(storedProvider);
        localStorage.removeItem('temp-subcategories-values');
    }
...
<svelte:head>
    <title>Detalle Subcategoría</title>
</svelte:head>
<SubcategoriesEdit {tempId} {filterName} {filterValue} values={values}  />