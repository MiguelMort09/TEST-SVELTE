<script lang="ts">
  ...
  import { categoriesStore, addCategories } from './categories';
  ...
  let rows:any[] = [];
  let allRows:CategoriesType[] = [];
  let valueObject:any;
  let addForm:HTMLDivElement;
  let unsubscribe:Unsubscriber|undefined;

  let categories:Readable<CategoriesType[]>|undefined;
  ...
  // }
    ...
    unsubscribe = categories.subscribe(data => {
      allRows = data;
      //sort by name
  ...
  async function addOption(){
    Edit = (await import('$lib/pages/categories/CategoriesEdit.svelte')).default;
    await swal({
  ...
  function onBack(cancel?:boolean, data?:CategoriesType){
...
<div class="{withStyle?'form-group flex':''}">
  {#if inlist && $categories != undefined}
