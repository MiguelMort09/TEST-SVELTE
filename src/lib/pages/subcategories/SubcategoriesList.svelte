<script lang="ts">
    ...
    import { SubcategoriesSchema } from '$lib/controls/types/subcategories';
    import { deleteSubcategories, duplicateSubcategories, SubcategoriesFromData } from '$lib/controls/subcategories';
    
    //import SubcategoriesEdit from "./SubcategoriesEdit.svelte";
    ...
    export let title="Administración Subcategorias";
    export let filterName:string|undefined = undefined;
    export let filterValue:string|undefined = undefined;
    export let filters:{[key:string]:(item:SubcategoriesType)=>boolean} = {};
    ...
    $: filters, subcategories, filterData();
    function filterData(){
        let filtered:SubcategoriesType[] = [];
        ...
        if(grid != undefined){
            grid.updateConfig({data:filteredSubcategories});
        }
    }
    ...
    if(profileFilter != null){
        unsubscribeProfile = profile.subscribe((value:any) => {
            ...
            if(!$permissions.admin && !$permissions.subcategories){
        ...
        //if(!loadDataTable){
			...
			{ id: 'categoryName', name:'Categoría',width:"auto" },
				...
				{ id: 'name', name:'Nombre',width:"auto", formatter: function (data:any, type:any, row:any, meta:any) {
                    ...
                    return html('<a href="/subcategories/edit/'+id+'">'+data+'</a>');
                }, },
			{ id: 'id', sort:false, hidden:!hasActions, name:'Acciones', formatter: function (data:any, type:any, row:any, meta:any) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons.push(html('<a href="/subcategories/edit/'+data+'" class="btn btn-light waves-effect h-10"><iconify-icon icon="uil:edit" class="text-lg align-middle"></iconify-icon></a>'));
						}
					}
					
					buttons.push(h('button', {
						className:"btn btn-outline-danger h-10",
							onClick: () => deleteSubcategories( data)
	...
	function loadFirebaseData(){
        let collRef:CollectionReference|Query  = collection(db,subcolLink + "subcategories");
        ...
        unsubscribe = onSnapshot(collRef, async (querySnapshot) => {
            ...
            for (let d = 0; d < querySnapshot.docs.length; d++) {
                const tempsubcategories = SubcategoriesFromData(querySnapshot.docs[d].id, querySnapshot.docs[d].data());
...
function asyncChange(id:string, field:string, newval:any){
    ...
    var update:any = {};
    update[field] = newval;
    setDoc(doc(db,subcolLink + "subcategories", id),update, {merge:true});
}
...
interface Column {
  property: string;
  title: string;
  format?: (row: SubcategoriesType) => any;
}
...
function downloadExcel(){
    let cols:Column[] = [
        ...
        {property:"added",title:"Fecha de Creación",format:(row:SubcategoriesType) => {
    ...
    createAndDownloadExcel(subcategories, cols);
}


async function createAndDownloadExcel<SubcategoriesType>(tmp: SubcategoriesType[], cols: Column[]): Promise<void> {
    ...
    for (let d = 0; d < tmp.length; d++) {
        const tempsubcategories = tmp[d];
        			
			categories.toQueue(tempsubcategories.category);
				

        tmp[d] = tempsubcategories;
    }
...
<div class="page-content">
    ...
    <!-- <TitleBar title="Subcategorias" base="Inventario" /> -->
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                        ...
                        {:else if canView}
                            <div class="flex gap-2">
                                ...
                                <div class="flex">
                                    ...
                                    {#if modal}
                                    <button type="button" class="btn btn-primary h-10 rounded-none rounded-r-lg " on:click={()=>{ setEditId(undefined);}}> Agregar Subcategoría</button>
                                    {:else}
                                    <button on:click={function() { 
                                        let s = subcolLink.replace(/\/$/, ''); 
                                        if(filterName != undefined && filterValue != undefined){
                                            goto(s+'/subcategories/edit/'+filterName+'/'+filterValue+"/");
                                        }else{
                                            goto(s+'/subcategories/edit/');
                                        }
                                        
                                    }} class="btn btn-add-record h-10 rounded-none rounded-r-lg ">
                                        <iconify-icon
                                        icon="ic:round-add" class="md:mr-2 text-xl"
                                    ></iconify-icon> <span class="hidden md:block">Agregar Subcategoría</span></button>
                                    {/if}
                                </div>
                            </div>
                            <div class="flex gap-2">
                ...
                <Grid bind:instance={grid} data={filteredSubcategories} language={esES} columns={columns} search={canView} sort pagination={{enabled:true,limit:20}} autoWidth={true} style={{table: { 
