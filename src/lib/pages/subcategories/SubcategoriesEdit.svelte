<script lang="ts">
  ...
  import { SubcategoriesFromData, subcategoriesGetAll, subcategoriesGetNextOfField } from '$lib/controls/subcategories';
 ...
 async function checkVisibles(){
  ...
  export let onBack = (cancel?:boolean, data?:SubcategoriesType)=> {
    let defaultRoute = '/subcategories';
  ...
  onMount(() => {
    ...
    if (tempId == null) {
      ...
      getDoc(doc(db, "subcategories", tempId)).then(function(query) {
  ...
  async function updateData() {
    ...
    if (tempId != null) {
      ...
      if(!exists){
        ...
        setDoc(doc(db, "subcategories",tempId) , values).then(function () {

            loading = false;

            let data:SubcategoriesType|undefined = undefined;
            try{
              data = SubcategoriesFromData(tempId, values);
            }catch(e){
              console.log(e);
            }
            onBack(false,data);
        ...
        updateDoc(doc(db, "subcategories",tempId) , values).then(function() {
            loading = false;
            let data = SubcategoriesFromData(tempId!, values);
            onBack(false,data);
          });
      }
      ...
      values.createdBy = uid;
      values.createdName = userName;
      let newDoc = doc(collection(db, "subcategories"));
      let dId = newDoc.id;
      ...
      setDoc(doc(db, "subcategories",dId) , values).then(function () {

          loading = false;
          let data = SubcategoriesFromData(dId, values);
          onBack(false, data);
        });
    }
  }
  ...
  async function autoIncrement(field:string):Promise<number>{
    let maxDocs = await subcategoriesGetNextOfField( field);
    return maxDocs;
  }
</script>

<div class="page-content modal-body w-full max-w-2xl mx-auto ">
  <!-- <TitleBar title="Agregar Subcategoría" base="Inventario" /> -->
<form id="form" bind:this={form}>
  <div class="row">
    {#if viewTitle}
      <h4 class="card-title mt-4">Subcategoría</h4>
    {/if}
