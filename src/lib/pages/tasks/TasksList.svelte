<script lang="ts">
    ...
    import { TasksSchema } from '$lib/controls/types/tasks';
    import { deleteTasks, duplicateTasks, TasksFromData } from '$lib/controls/tasks';
    
    //import TasksEdit from "./TasksEdit.svelte";
...
import {subcategoriesValuesMapById} from "$lib/controls/subcategories"
    ...
    $: filters, tasks, filterData();
    function filterData(){
        let filtered:TasksType[] = [];
        ...
        if(grid != undefined){
            grid.updateConfig({data:filteredTasks});
        }
    }
    ...
    if(profileFilter != null){
        unsubscribeProfile = profile.subscribe((value:any) => {
            ...
            if(!$permissions.admin && !$permissions.tasks){
        ...
        //if(!loadDataTable){
            columns = [
				{ id: 'name', name:'Nombre',width:"auto", formatter: function (data:any, type:any, row:any, meta:any) {
                    ...
                    return html('<a href="/tasks/edit/'+id+'">'+data+'</a>');
                }, },
			{ id: 'completed', name:'Completado',width:80, formatter: function (data:any, type:any, row:any, meta:any) {
                    return data === true
                        ? 'Si'
                        : 'No';
                }, },
			{ id: 'categoryName', name:'Categoría',width:"auto" },
			{ id: 'subcategoryName', name:'Subcategoría',width:"auto" },

							{ id: 'dueDate', name:'Fecha Compromiso',width:"auto", formatter: function (data:any, type:any, row:any, meta:any) {
								...
								let date;
								 if (data instanceof Date){
									date = DateTime.fromJSDate(data).setLocale("es-mx");
								 }else{
									date = DateTime.fromJSDate(data.toDate()).setLocale("es-mx");
								 }
			...
			{ id: 'id', sort:false, hidden:!hasActions, name:'Acciones', formatter: function (data:any, type:any, row:any, meta:any) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons.push(html('<a href="/tasks/edit/'+data+'" class="btn btn-light waves-effect h-10"><iconify-icon icon="uil:edit" class="text-lg align-middle"></iconify-icon></a>'));
						}
					}
					
					buttons.push(h('button', {
						className:"btn btn-outline-danger h-10",
							onClick: () => deleteTasks( data)
	...
	function loadFirebaseData(){
        let collRef:CollectionReference|Query  = collection(db,subcolLink + "tasks");
        ...
        unsubscribe = onSnapshot(collRef, async (querySnapshot) => {
            ...
            for (let d = 0; d < querySnapshot.docs.length; d++) {
                const temptasks = TasksFromData(querySnapshot.docs[d].id, querySnapshot.docs[d].data());
...
function asyncChange(id:string, field:string, newval:any){
    ...
    var update:any = {};
    update[field] = newval;
    setDoc(doc(db,subcolLink + "tasks", id),update, {merge:true});
}
...
function downloadExcel(){
    let cols:Column[] = [
		...
		{title: 'Completado', property: 'completed', },
		{title: 'Id Categoría', property: 'category', },
		{title: 'Categoría', property: 'categoryName', },
		{title: 'Id Subcategoría', property: 'subcategory', },
		{title: 'Subcategoría', property: 'subcategoryName', },
		{title: 'Fecha Compromiso', property: 'dueDate', },

        {property:"added",title:"Fecha de Creación",format:(row:TasksType) => {
    ...
    createAndDownloadExcel(tasks, cols);
}


async function createAndDownloadExcel<TasksType>(tmp: TasksType[], cols: Column[]): Promise<void> {
...
<div class="page-content">
    ...
    <!-- <TitleBar title="Tareas" base="Inventario" /> -->
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                        ...
                        {:else if canView}
                            <div class="flex gap-2">
                                ...
                                <div class="flex">
                                    ...
                                    {#if modal}
                                    <button type="button" class="btn btn-primary h-10 rounded-none rounded-r-lg " on:click={()=>{ setEditId(undefined);}}> Agregar Tarea</button>
                                    {:else}
                                    <button on:click={function() { 
                                        let s = subcolLink.replace(/\/$/, ''); 
                                        if(filterName != undefined && filterValue != undefined){
                                            goto(s+'/tasks/edit/'+filterName+'/'+filterValue+"/");
                                        }else{
                                            goto(s+'/tasks/edit/');
                                        }
                                    ...
                                    ></iconify-icon> <span class="hidden md:block">Agregar Tarea</span></button>
                                    {/if}
                                </div>
                            </div>
                            <div class="flex gap-2">
                ...
                <Grid bind:instance={grid} data={filteredTasks} language={esES} columns={columns} search={canView} sort pagination={{enabled:true,limit:20}} autoWidth={true} style={{table: { 
